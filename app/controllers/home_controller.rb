...
class HomeController < ApplicationController
	def index
		...
		@tweets = @active_squad.tweets.select('DISTINCT(text), positive, date').where("text NOT LIKE ?", '%RT %').order('created_at DESC').limit(10)
	...
	def show_tweets
		...
		@tweets = @player.tweets.select('DISTINCT(text), positive, date').where("text NOT LIKE ?", '%RT %').order('created_at DESC').limit(10)
